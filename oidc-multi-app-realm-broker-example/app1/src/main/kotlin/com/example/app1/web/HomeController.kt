...
class HomeController(
    ...
    private fun populateHomeModel(
        ...
        model.addAttribute(
            ...
            when {
                ...
                oidcUser != null ->
                    "이 계정은 현재 앱 전용 계정입니다. 아래 권한 매트릭스에서 멀티앱 계정 또는 ${app1ViewProperties.peerAppName} 관리자 계정을 확인해보세요."
                !from.isNullOrBlank() ->
    ...
    private fun accountInsight(
        ...
        return when {
            ...
            username == MULTI_USER || authorities.contains("ROLE_app1-user") && authorities.contains("ROLE_app2-user") -> AccountInsight(
                ...
                nextStep = "Peer 앱으로 이동해 agency-b realm이 platform-broker 세션을 재사용하는지 확인하세요.",
            )
            canManageCurrentApp -> AccountInsight(
                tone = "manager",
                label = "app-admin",
                title = "${app1ViewProperties.appName} 관리자 계정",
                detail = "agency-a realm에서만 관리자 role을 가진 계정입니다. App 1 세션은 관리할 수 있지만 agency-b realm에는 운영 권한이 없어 peer 앱에서는 접근이 거부됩니다.",
                currentAppAccess = "${app1ViewProperties.appName} 접근 가능",
                ...
                label = "agency-a-user",
                title = "현재 앱 전용 사용자",
                detail = "agency-a realm의 local role만 가진 계정입니다. platform-broker 인증은 성공하지만 peer 앱으로 이동하면 agency-b realm의 인가 단계에서 차단됩니다.",
                currentAppAccess = "${app1ViewProperties.appName} 접근 가능",
    ...
    private fun ssoJourney(): List<JourneyStep> {
        return listOf(
            ...
            JourneyStep("2", "현재 앱 인증", "agency-a realm이 요청을 받고 `kc_idp_hint=platform-broker`로 공통 broker realm 로그인을 시작합니다."),
            JourneyStep("3", "원클릭 전환", "아래 자동 전환 버튼 또는 바로가기 버튼으로 ${app1ViewProperties.peerAppName} 인증을 이어가면 agency-b realm이 같은 broker 세션을 재사용합니다."),
            JourneyStep("4", "권한 차이 확인", "`agency-a-user`, `agency-b-user`, `agency-a-admin`으로 403과 관리자 범위를 비교해볼 수 있습니다."),
        )
    }
