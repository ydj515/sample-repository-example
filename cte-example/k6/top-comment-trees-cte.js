...
export const options = {
    ...
    thresholds: {
        ...
        cte_service_duration: ['p(95)<300'],
    },
};

const cteServiceDuration = new Trend('cte_service_duration');

export default function () {
    const limit = nextLimit();
    const response = http.get(
        `${BASE_URL}/api/posts/${POST_ID}/comments/top-trees/cte?limit=${limit}`,
        {tags: {endpoint: 'cte'}}
    );
    ...
    if (payload && payload.durationMillis !== undefined) {
        cteServiceDuration.add(payload.durationMillis);
    }
}
