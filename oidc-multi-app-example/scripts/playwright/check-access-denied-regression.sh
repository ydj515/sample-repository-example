...
check_http_ready "${KEYCLOAK_BASE_URL}/realms/oidc-multi-app-example/.well-known/openid-configuration" "Keycloak"

echo "접근 거부 회귀 점검을 시작합니다."
echo "결과 폴더: ${RUN_DIR}"

run_scenario \
    "app1-denied-by-app2-user" \
    "${APP1_BASE_URL}" \
    "app2-user" \
    "app2user1234" \
    "App 1 Access Denied" \
